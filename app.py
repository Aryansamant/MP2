...
with left_col:
    ...
    selected_customer = st.selectbox("Select a customer", customers) if customers else None
...
if st.button("Ask AI"):
    if not nl_question.strip():
        ...
        sql_from_ai = sql_from_ai.strip()

        # Remove triple backticks: ```sql ... ```
        if sql_from_ai.startswith("```"):
            sql_from_ai = sql_from_ai.replace("```", "")
            sql_from_ai = sql_from_ai.replace("sql", "")
            sql_from_ai = sql_from_ai.strip()

        # Remove leading "sql " prefix
        if sql_from_ai.lower().startswith("sql "):
            sql_from_ai = sql_from_ai[4:].strip()

        # Remove leading "sql" prefix (no space)
        if sql_from_ai.lower().startswith("sql"):
